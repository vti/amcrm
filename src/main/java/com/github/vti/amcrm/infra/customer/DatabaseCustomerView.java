...
public class DatabaseCustomerView implements CustomerView {
    ...
    public Optional<CustomerSummary> loadForAdmin(String id) {
        try (Connection connection = this.dataSource.getConnection()) {
            ...
            Record5<String, String, String, String, String> record =
                    create.select(
                                    CUSTOMER.ID,
                                    CUSTOMER.NAME,
                                    CUSTOMER.SURNAME,
            ...
            if (record == null) {
                ...
                CustomerSummary customer =
                        CustomerSummary.builder()
                                ...
                                .surname(record.getValue(CUSTOMER.SURNAME))
                                .photoLocation(record.getValue(CUSTOMER.PHOTO_LOCATION))
                                .createdBy(record.getValue(CUSTOMER.CREATED_BY))
                                .build();
                return Optional.of(customer);
            }
        } catch (SQLException e) {
    ...
    public Page<CustomerSummary> findForAdmin(Pager pager) {
        ...
        try (Connection connection = this.dataSource.getConnection()) {
            ...
            for (Record5<String, String, String, String, String> record : result) {
                CustomerSummary customer =
                        CustomerSummary.builder()
                                .baseUrl(baseUrl)
                                .id(record.getValue(CUSTOMER.ID))
